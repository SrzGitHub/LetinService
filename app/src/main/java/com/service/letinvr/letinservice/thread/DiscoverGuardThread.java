 ...
 **/
public class DiscoverGuardThread extends Thread {
    
    private final Timer timer;
    private TimerTask task;

    public DiscoverGuardThread(){
        timer = new Timer();
    }
    ...
    public void run() {
        task = new TimerTask() {
            ...
            public void run() {
                ...
                boolean b = MainActivity.isServiceWorked(APP.getContext(), API.SERVICE_TWO);
                if (!b) {//如果Service没有运行  那么就启动Service
                    Intent service = new Intent(APP.getContext(), LetinServers.class);
